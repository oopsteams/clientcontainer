...
var api = {
	...
	re_check_state_by_pan_acc:function(token, point, parent_win, cb){
		call_pansite_by_post(point, "man/pan_acc_list", {token: token}, parent_win, (json_obj)=>{
			pan_acc_list = json_obj.pan_acc_list;
			auth_redirect = json_obj.auth;
			pan_acc_list.forEach((pa, idx)=>{pa['token'] = token});
			api.loop_check_accounts(token, point, pan_acc_list, auth_redirect, parent_win, (isok)=>{
				if(cb){cb(isok);}
			});
		});
	},
...
var createCheckWindow =(pan_acc, token, point, redirect_url, parent_win, callback, only_check) => {
	if(checkWindow == null){
	...
	if(!only_check)only_check = false;
	var pan_name = '';
