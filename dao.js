			...
			function save_by_check(pos, tag){
				...
				if(_item && _item.hasOwnProperty('id')){
					self.get('id', _item.id, (_get_item)=>{
						...
						if(!_get_item){
							self.put(_item, ()=>{
								setTimeout(()=>{save_by_check(pos+1, 'new item with id!');},10);
							});
						}else{
							if(conflict_check_cb){
								conflict_check_cb(_item, _get_item, (update_it)=>{
									if(update_it){
										if(update_it.hasOwnProperty('id')){delete update_it['id'];}
					...
					self.put(_item, ()=>{
						setTimeout(()=>{save_by_check(pos+1, 'new item without id!');},1);
					});
				} else {
