...
const item_id_field = {name:"item_id", type:'VARCHAR', len:16};
...
'fields':[char_id_field, created_at_field,
	...
	{name:"pin", type:'INT'},
	{name:"rdlink", type:'VARCHAR', len:1024},
	{name:"content_type", type:'VARCHAR', len:32},
	...
	{name:"source_id", type:'VARCHAR', len:20 , index:true},
	{name:"used", type:'INT'}
	]
});
...
var nsSubTask = Base.extend({
	...
	check_req_stream_file:function(file_path, callback){
		setTimeout(()=>{
			if(fs.existsSync(file_path)){
				...
				if(f_size == 0){
					var rs = {'error_code':9999, 'error_msg':'nothing in this file['+file_path+']!'};
					if(callback){
						callback(rs);
					}
					return rs;
				...
				if(this.size() > f_size){
					rm_fs();
					var rs = {'error_code':9999, 'error_msg':'datas is not enough, in this file['+file_path+']!'};
					if(callback){
						callback(rs);
					}
					return rs
			...
			ithis.update_state(0, ()=>{
			  if(check_rs.hasOwnProperty('error_code')){
				  ...
				  if(31045 == check_rs.error_code){
					  ...
					  download_loader_db.update_by_conditions_increment({'id':loader.id}, {'pin': new_loader_pin}, {'used':-1},()=>{
						  download_sub_task_db.update_by_id(ithis.params['id'], {'loader_id':0},()=>{
							  ithis.params['loader_id'] = 0;
							ithis.update_state(3,()=>{
								ithis.recheck_loader(loader,()=>{
	...
	compute_progress:function(){
		...
		return {'prog_val':prog_val, 'exhaust': self.exhaust, 'speed':self.speed, 'get_size':get_size, 'id': this.params['id'], 'start': start};
	}
});
////////////////////////////////////////////////NS TASK CLASS
var nstask = Base.extend({
		...
		var query_file_head_callback = function(url, params){
			...
			var build_sub_task = ()=>{
				...
				for(i=0;i<page_count-1;i++){
					...
					var task_params = {'id':ithis.task.id+'_'+i, 'idx':i, 'source_id':ithis.task.id, 'start':i*page_size, 'end':(i+1)*page_size, 'over':0, 'retry':0, 'loader_id': loader.id, 'state': 0};
	...
	_update_rdlink:function(loader,callback){
		...
		if(!loader.rdlink || loader.rdlink.length<=1){
			ithis.nsloader.query_file_head(loader['dlink'], (rdlink, params)=>{
				if(rdlink){
					...
					download_loader_db.update_by_id(loader['id'], {
						'rdlink': rdlink, 'size': content_length, 'content_type': content_type
					}, ()=>{
			...
			var params = {rdlink:url, type:content_type, length:content_length}
			callback(url, params);
		}
	},
	init_dlink:function(callback){
		...
		download_loader_db.update_by_conditions({'source_id':self.task.id}, {'pin': 0}, function(){
	...
	update_type:function(type, cb){
		...
		download_task_db.update_by_id(this.task['id'], {'type': type}, (_id, _params)=>{
	...
	checkout_loader_list:function(cb){
		...
		download_loader_db.query_mult_params({'source_id':self.task.id}, (loader_list)=>{
			if(cb){
				cb(loader_list);
			}
			...
			if(t.get_state() == 7){ //处理一下 state:3中途失败的段
				...
				var renew_sub_task = ()=>{
					...
					if(total_size >= ithis.section_min_size * 2){
						...
						new_task_params_list.push({'id':new_id_prefix + suffix, 'source_id':t.params.source_id, 'start':new_start, 'end':t.params.end, 'over':0, 'idx':2, 'retry':0, 'loader_id': _sub_t_loader_id, 'state': 0, 'patch': 0});
					}
					var new_task_list = [];
	...
	valid_tasks_order:function(){
		...
		for(var i=0;i<all_sub_tasks.length;i++){
			...
			if(pos == start_at){
				...
				console.log('valid_tasks_order failed, valid pos:', pos, ',task start pos:', start_at, ',last_id:',last_id, ',id:', t.params["id"]);
				return null;
			}
		}
							...
							ithis.recover_sub_task(task_params, (_rs, nst)=>{
								...
								t_a.update_pos(t_a.params.start, t_a.params.start,()=>{
									t_b.update_pos(t_b.params.start, t_b.params.start,()=>{
										t_a.del(()=>{
											t_b.del(()=>{
												try{fs.unlinkSync(a_file_path);}catch(e){}
												try{fs.unlinkSync(b_file_path);}catch(e){}
				...
				if(states.size>0){
					if(t.size() == states.size){
						...
						// } else {
							...
							var task_params = {'id':id_prefix+'_'+pos, 'source_id':t.params.source_id, 'start':new_start, 'end':t.params.end, 'over':0, 'idx':t.params.idx, 'retry':0, 'loader_id': t.params.loader_id, 'state': 0};
							ithis.recover_sub_task(task_params, (_rs, nst)=>{
								async_cnt += 1;
								t.update_pos(t.params.start, new_start, ()=>{
									t.update_state(2,()=>{
				...
				if(loader_list && loader_list.length>0){
					...
					if(ids.length>0){
						self.nsloader.account.check_state((isok, rs)=>{
							...
							call_pansite_by_post(rs.tk, ithis.point, _path, {"ids": ids.join(','),"useds":useds.join(',')}, (result)=>{
			...
			if(file_dirs && file_dirs.length>0 && fs.existsSync(file_dirs[0])){
				...
				if(!fs.existsSync(new_file_path)){
					...
					if(fs.existsSync(final_file)){
						ithis.nsloader.cfg.update('default_save_path', file_dir, 'last_open_dir', (p)=>{
							fs.rename(final_file, new_file_path, (err)=>{
								if(err){
									...
									ithis.nsloader.send({'tag':'alert', 'msg': err_info});
									// if(cb){cb(1);}
									final_call(1);
								} else {
									// if(cb){cb(0);}
						...
						ithis.nsloader.send({'tag':'alert', 'msg': '文件['+ithis.task.filename+']已经不存在!'});
					}
				} else {
					ithis.nsloader.send({'tag':'alert', 'msg': '存在同名文件,迁移失败!'});
					// if(cb){cb(1);}
					final_call(1);
				}
			} else {
	...
	compute_progress:function(){
		...
		return {'file_url':file_url,'prog_val': prog_val, 'exhaust':self.exhaust, 'speed':self.speed,
		'sub':sub_task_list, 'task':self.task, 'total_length':total_length, 'get_size':self.get_download_complete_size()};
	}
});
/////////////////////////////////////////////////// NS LOADER
var nsloader = Base.extend({
	...
	query_file_head:function(url, callback, ua){
		var self = this;
		this.nsproxy.fetch_real_dlink({'dlink':url, 'ua':ua}, (rs, item, params)=>{
			if(rs){
	...
	_parse_task_state:function(nst, callback){
		...
		if(nst.get_state() == 0){
			...
			result = {'state': -101, 'pos': pos, 'err': '网络访问故障!'};
			callback(false, result);
		} else {
			result = {'state': 0, 'pos': pos};
			callback(false, result);
